...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        ...
        // Последовательный запуск двух параллельных задач
        QtConcurrent::run(&ExampleRace::DoWork, concurRace1, &number, mutexOn, iterations);
        QtConcurrent::run(&ExampleRace::DoWork, concurRace2, &number, mutexOn, iterations);
